...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError("Le dossier local doit être un chemin absolu complet.");
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError("Le dépôt doit utiliser une URL GitHub ou GitHub Enterprise valide.");
      return;
    }
      ...
      >
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">URL du dépôt</label>
              <span className="text-xs text-muted-foreground/50">optionnel</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  Liez un dépôt GitHub pour que les agents puissent cloner, lire et pousser du code pour ce projet.
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">Dossier local</label>
              <span className="text-xs text-muted-foreground/50">optionnel</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  Définissez un chemin absolu sur cette machine où les agents locaux liront et écriront des fichiers pour ce projet.
                </TooltipContent>
              </Tooltip>
            </div>
            ...
            <PopoverContent className="w-56 p-1" align="start">
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  Tous les objectifs sont déjà sélectionnés.
                </div>
              )}
            </PopoverContent>
          </Popover>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">Échec de la création du projet.</p>
          ) : (
            <span />
          )}
          <Button
