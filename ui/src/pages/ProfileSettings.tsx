...
export function ProfileSettings() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "Échec de la mise à jour du profil.");
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error("Sélectionnez une entreprise avant de téléverser un avatar.");
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "Échec du téléversement de l'avatar.");
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "Échec de la suppression de l'avatar.");
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : "Échec du chargement du profil."}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? `Stocké dans Paperclip pour ${selectedCompany.name}.`
    : "Sélectionnez une entreprise pour téléverser un avatar dans Paperclip.";
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Contrôlez l'apparence de votre compte dans la barre latérale et les autres surfaces.
        </p>
      </div>
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? "Changer la photo" : "Téléverser une photo"}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? "Aucun e-mail"}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  Cliquez sur l'avatar pour téléverser une nouvelle image. {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Affiché dans le pied de page de la barre latérale et sur les commentaires.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              L'e-mail est géré par votre session d'authentification et est en lecture seule ici.
            </p>
          </div>
