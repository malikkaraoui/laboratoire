  ...
  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? "Vous n'avez pas la permission de gérer les membres de l'entreprise."
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : "Échec du chargement des membres de l'entreprise.";
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Accès à l'entreprise</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          Gérez les adhésions, les statuts et les autorisations explicites des utilisateurs de l'entreprise {selectedCompany?.name}.
        </p>
      </div>

      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          Ce compte peut gérer les accès ici via les privilèges d'administrateur d'instance, mais il ne détient pas actuellement de membre actif dans l'entreprise.
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">
            Gérez ici les adhésions, statuts et autorisations des membres humains de l'entreprise.
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Examinez les demandes d'adhésion humaines avant qu'elles deviennent membres actifs.
                </p>
              </div>
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          ...
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">Aucune adhésion trouvée pour cette entreprise pour l'instant.</div>
          ) : (
            members.map((member) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Modifiez le rôle, le statut d'adhésion et les autorisations explicites de {editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId}.
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-sm font-medium">Autorisations</h3>
                  <p className="text-sm text-muted-foreground">
                    Les rôles fournissent des autorisations implicites automatiquement. Les autorisations explicites ci-dessous sont uniquement pour les remplacements et accès supplémentaires devant persister même si le rôle change.
                  </p>
                </div>
                <div className="rounded-lg border border-border px-3 py-3">
                  <div className="text-sm font-medium">Autorisations implicites du rôle</div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {draftRole
                      ? `${HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole]} inclut actuellement ces permissions automatiquement.`
                      : "Aucun rôle sélectionné, ce membre n'a donc pas d'autorisations implicites pour l'instant."}
                  </p>
                      ...
                      <span className="space-y-1">
                        ...
                        {implicitGrantSet.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            Inclus implicitement par le rôle {draftRole ? HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] : "sélectionné"}. Ajoutez une autorisation explicite uniquement si elle doit persister après un changement de rôle.
                          </span>
                        ) : null}
                        {draftGrants.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            Stocké explicitement pour ce membre.
                          </span>
                        ) : null}
                      </span>
                    </label>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending ? "Enregistrement…" : "Enregistrer les accès"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
              ...
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? "Vérification des tickets assignés..."
                    : `${assignedIssues.length} ticket${assignedIssues.length === 1 ? "" : "s"} ouvert${assignedIssues.length === 1 ? "" : "s"} assigné${assignedIssues.length === 1 ? "" : "s"}`}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">Réassignation des tickets</div>
                  <select
          ...
          <DialogFooter>
            ...
            >
              {archiveMemberMutation.isPending ? "Retrait en cours..." : "Retirer le membre"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
