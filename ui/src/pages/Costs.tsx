  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">Grand livre financier</CardTitle>
        <CardDescription>
          Charges au niveau du compte qui ne correspondent pas à une seule requête d'inférence.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={`${eventCount} événement${eventCount === 1 ? "" : "s"} au total dans la période`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label="Crédits"
          value={formatCents(creditCents)}
          subtitle="Remboursements, compensations et retours de crédit"
          icon={ArrowDownLeft}
        />
        <MetricTile
          label="Net"
          value={formatCents(netCents)}
          subtitle="Débits moins crédits pour la période sélectionnée"
          icon={ReceiptText}
        />
        <MetricTile
          label="Estimé"
          value={formatCents(estimatedDebitCents)}
          subtitle="Débits estimés non encore définitifs sur facture"
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="Sélectionnez une entreprise pour voir les coûts." />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Coûts</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  Dépenses d'inférence, frais de plateforme, crédits et fenêtres de quota en direct.
                </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label="Dépenses d'inférence"
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={`${formatTokens(inferenceTokenTotal)} tokens sur les événements liés aux requêtes`}
              icon={DollarSign}
            />
            <MetricTile
              label="Budget"
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} agent(s) mis en pause · ${budgetData?.pausedProjectCount ?? 0} projet(s) mis en pause`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? `${formatCents(spendData.summary.spendCents)} sur ${formatCents(spendData.summary.budgetCents)}`
                    : "Aucun plafond mensuel configuré"
              }
              icon={Coins}
            />
            <MetricTile
              ...
              subtitle={`${formatCents(financeData?.summary.debitCents ?? 0)} débits · ${formatCents(financeData?.summary.creditCents ?? 0)} crédits`}
              icon={ReceiptText}
            />
            <MetricTile
              label="Événements financiers"
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={`${formatCents(financeData?.summary.estimatedDebitCents ?? 0)} estimés dans la période`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="billers">Factureurs</TabsTrigger>
          <TabsTrigger value="finance">Finance</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">Sélectionnez une date de début et de fin pour charger les données.</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">Grand livre d'inférence</CardTitle>
                    <CardDescription>
                      Dépenses d'inférence liées aux requêtes pour la période sélectionnée.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 px-5 pb-5 pt-2">
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      ...
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">utilisation</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% du budget mensuel consommé dans cette période.
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">Par agent</CardTitle>
                    <CardDescription>Ce que chaque agent a consommé durant la période sélectionnée.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">Aucun événement de coût pour l'instant.</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">Par projet</CardTitle>
                      <CardDescription>Coûts d'exécution attribués via les tickets liés aux projets.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Aucun coût d'exécution attribué à un projet pour l'instant.</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? "Non attribué"}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="Aucun événement financier pour l'instant. Ajoutez des charges au niveau du compte dès réception des factures ou crédits." />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">Panneau de contrôle des budgets</CardTitle>
                  <CardDescription>
                    Plafonds de dépenses stricts pour les agents et les projets. Le quota d'abonnement fournisseur reste séparé et apparaît sous Fournisseurs.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle="Dépassements de seuil doux ou strict ouverts"
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label="Approbations en attente"
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle="Approbations de dérogation budgétaire en attente du conseil"
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label="Agents en pause"
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle="Heartbeats d'agents bloqués par le budget"
                    icon={Coins}
                  />
                  <MetricTile
                    label="Projets en pause"
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle="Exécution de projets bloquée par le budget"
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">Incidents actifs</h2>
                    <p className="text-sm text-muted-foreground">
                      Résolvez les arrêts stricts ici en augmentant le budget ou en maintenant explicitement la portée en pause.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "Politique mensuelle à l'échelle de l'entreprise."
                            : scopeType === "agent"
                              ? "Politiques de dépenses mensuelles récurrentes pour les agents individuels."
                              : "Politiques de dépenses à vie pour les projets liés à l'exécution."}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      Aucune politique budgétaire pour l'instant. Définissez les budgets des agents et des projets depuis leurs pages de détail, ou utilisez le contrôle budgétaire mensuel de l'entreprise existant.
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Aucun événement de coût dans cette période.</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Aucun événement facturable dans cette période.</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">Par factureur</CardTitle>
                      <CardDescription>Événements financiers au niveau du compte regroupés par émetteur de charge ou de crédit.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Aucun événement financier pour l'instant.</p>
                      ) : (
