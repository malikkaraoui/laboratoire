        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused" || isDraft) ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? "archivée" : isDraft ? "brouillon" : "en pause"}
            </span>
          ) : null}
        </div>
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={enabled ? `Désactiver ${routine.title}` : `Activer ${routine.title}`}
          />
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? "Archivée" : isDraft ? "Brouillon" : enabled ? "Activée" : "Désactivée"}
          </span>
        </div>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={`Plus d'actions pour ${routine.title}`}>
              <MoreHorizontal className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link to={href}>Modifier</Link>
            </DropdownMenuItem>
            <DropdownMenuItem
              disabled={runningRoutineId === routine.id || isArchived}
              onClick={() => onRunNow(routine)}
            >
              {runningRoutineId === routine.id ? "En cours..." : "Exécuter maintenant"}
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
            ...
            >
              {routine.status === "archived" ? "Restaurer" : "Archiver"}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        ...
        body: routine.assigneeAgentId
          ? "Ajoutez le premier déclencheur pour en faire un workflow actif."
          : "Brouillon enregistré. Ajoutez un agent par défaut avant d'activer l'automatisation.",
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Paperclip n'a pas pu mettre à jour la routine.",
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Paperclip n'a pas pu démarrer l'exécution de la routine.",
        tone: "error",
      });
    },
  });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Définitions de travaux récurrents qui se matérialisent en tickets d'exécution auditables.
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            ...
            { value: "runs", label: "Exécutions récentes" },
          ]}
        />
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">Nouvelle routine</p>
              <p className="text-sm text-muted-foreground">
                Définissez d'abord le travail récurrent. Le projet et l'agent par défaut sont facultatifs pour les routines en brouillon.
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">Paramètres de livraison avancés</p>
                    <p className="text-sm text-muted-foreground">Gardez les contrôles de politique secondaires par rapport à la définition du travail.</p>
                  </div>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              Après la création, Paperclip vous amène directement à la configuration des déclencheurs. Les routines en brouillon restent en pause jusqu'à ce que vous ajoutiez un agent par défaut.
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : "Échec de la création de la routine"}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : "Échec du chargement des routines"}
          </CardContent>
        </Card>
      ) : null}
        ...
        <div>
          {(routines ?? []).length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message="Aucune routine pour l'instant. Utilisez Créer une routine pour définir le premier workflow récurrent."
              />
            </div>
          ) : (
