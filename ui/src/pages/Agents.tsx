...
export function Agents() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message="Sélectionnez une entreprise pour voir les agents." />;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{filtered.length} agent{filtered.length !== 1 ? "s" : ""} au total</p>
      )}
      ...
      {agents && agents.length === 0 && (
        <EmptyState
          icon={Bot}
          message="Créez votre premier agent pour commencer."
          action="Nouvel agent"
      ...
      {effectiveView === "list" && agents && agents.length > 0 && filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          Aucun agent ne correspond au filtre sélectionné.
        </p>
      )}
      ...
      {effectiveView === "org" && orgTree && orgTree.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          Aucune hiérarchie organisationnelle définie.
        </p>
      )}
    </div>
  );
