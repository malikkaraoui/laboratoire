...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: "Le JSON des commandes d'espace de travail doit être un objet JSON.",
      };
    }
...
function validateForm(form: WorkspaceFormState) {
  ...
  if (repoUrl) {
    try {
      ...
      return "L'URL du dépôt doit être une URL valide.";
    }
  }
...
function MonoValue({ value, copy }: { value: string; copy?: boolean }) {
  return (
    <div className="inline-flex max-w-full items-start gap-2">
      ...
      {copy ? (
        <CopyText text={value} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel="Copié">
...
export function ExecutionWorkspaceDetail() {
  ...
  useEffect(() => {
    ...
    const crumbs = [
      ...
      ...(project ? [{ label: "Espaces de travail", href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : "Échec de la sauvegarde de l'espace de travail d'exécution.");
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? "Tâche d'espace de travail terminée."
          : request.action === "stop"
            ? "Service d'espace de travail arrêté."
            : request.action === "restart"
              ? "Service d'espace de travail redémarré."
              : "Service d'espace de travail démarré.",
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : "Échec du contrôle des commandes d'espace de travail.");
    },
  });

  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">Chargement de l'espace de travail…</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : "Échec du chargement de l'espace de travail"}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Échec de la construction de la mise à jour de l'espace de travail.");
      return;
    }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-wrap items-center gap-3">
          <Button variant="ghost" size="sm" asChild>
            <Link to={project ? `/projects/${projectRef}/workspaces` : "/projects"}>
              ...
              Retour à tous les espaces de travail
            </Link>
          </Button>
        ...
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            Configurez l'espace de travail d'exécution concret que Paperclip réutilise pour ce flux de ticket.
            <span className="hidden sm:inline"> Ces paramètres restent attachés à l'espace de travail d'exécution afin que les exécutions futures puissent garder les chemins locaux, les références de dépôt, le provisionnement, le démontage et le comportement des services d'exécution synchronisés avec l'espace de travail réel réutilisé.</span>
          </p>
        </div>

        <Card className="rounded-none">
          <CardHeader>
            <CardTitle>Services et tâches</CardTitle>
            <CardDescription>
              Source : {runtimeConfigSource === "execution_workspace"
                ? "remplacement de l'espace de travail d'exécution"
                : runtimeConfigSource === "project_workspace"
                  ? "valeur par défaut de l'espace de travail du projet"
                  : "aucune"}
            </CardDescription>
          </CardHeader>
          <CardContent>
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? "Aucun service n'a encore été démarré pour cet espace de travail d'exécution."
                : "Aucune configuration de commande d'espace de travail n'est définie pour cet espace de travail d'exécution."
            }
            jobEmptyMessage="Aucune tâche ponctuelle n'est configurée pour cet espace de travail d'exécution."
            disabledHint={
              canStartRuntimeServices
                ? null
                : "Les espaces de travail d'exécution nécessitent un répertoire de travail avant que les commandes locales puissent s'exécuter, et les services ont également besoin d'une configuration d'exécution."
            }
        ...
        <Tabs value={activeTab ?? "configuration"} onValueChange={(value) => handleTabChange(value as ExecutionWorkspaceTab)}>
          <PageTabBar
            items={[
              ...
              { value: "runtime_logs", label: "Journaux d'exécution" },
        ...
        {activeTab === "configuration" ? (
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>Paramètres de l'espace de travail</CardTitle>
                <CardDescription>
                  Modifiez le chemin concret, le dépôt, la branche, le provisionnement, le démontage et les remplacements d'exécution attachés à cet espace de travail d'exécution.
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? "Réessayer la fermeture" : "Fermer l'espace de travail"}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Général</div>
                  <Field label="Nom de l'espace de travail">
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder="Nom de l'espace de travail d'exécution"
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Contrôle de source</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="Nom de branche" hint="Utile pour les worktrees isolés">
                      <Input
                        className="font-mono"
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Chemins</div>
                  <Field label="Répertoire de travail">
                    <Input
                      className="font-mono"
                      value={form.cwd}
                      onChange={(event) => setForm((current) => current ? { ...current, cwd: event.target.value } : current)}
                      placeholder="/chemin/absolu/vers/workspace"
                    />
                  </Field>

                  <Field label="Chemin fournisseur / référence">
                    <Input
                      className="font-mono"
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Commandes de cycle de vie</div>
                  <Field label="Commande de provisionnement" hint="S'exécute lorsque Paperclip prépare cet espace de travail d'exécution">
                    <Textarea
                  ...
                  <Field label="Commande de démontage" hint="S'exécute lorsque l'espace de travail d'exécution est archivé ou nettoyé">
                    <Textarea
                      className="min-h-20 font-mono"
                  ...
                  <Field label="Commande de nettoyage" hint="Nettoyage spécifique à l'espace de travail avant le démontage">
                    <Textarea
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Configuration d'exécution</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          Source de la configuration d'exécution
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? "Cet espace de travail d'exécution remplace actuellement la configuration d'exécution de l'espace de travail du projet."
                            : runtimeConfigSource === "project_workspace"
                              ? "Cet espace de travail d'exécution hérite de la configuration d'exécution de l'espace de travail du projet."
                              : "Aucune configuration d'exécution n'est actuellement définie sur cet espace de travail d'exécution ou son espace de travail de projet."}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                  ...
                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">JSON d'exécution avancé</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      Remplacez le modèle de commande d'espace de travail hérité uniquement lorsque cet espace de travail d'exécution nécessite vraiment un comportement de service ou de tâche différent.
                    </p>
                    <div className="mt-3">
                      <Field label="JSON des commandes d'espace de travail" hint="Les tableaux `services` hérités fonctionnent encore, mais `commands` prend en charge les services et les tâches.">
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">Hériter la configuration d'exécution de l'espace de travail du projet</label>
                        </div>
                        <Textarea
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">Aucune modification non enregistrée.</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>Contexte de l'espace de travail</CardTitle>
                <CardDescription>Objets liés et relations</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <DetailRow label="Espace de travail du projet">
              ...
              <DetailRow label="ID de l'espace de travail">
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>Emplacement concret</CardTitle>
                <CardDescription>Chemins et références</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label="Répertoire de travail">
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : "Aucun"}
              </DetailRow>
              <DetailRow label="Référence fournisseur">
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : "Aucun"}
              </DetailRow>
              <DetailRow label="URL du dépôt">
                {workspace.repoUrl && isSafeExternalUrl(workspace.repoUrl) ? (
                  <div className="inline-flex max-w-full items-start gap-2">
                    ...
                    <CopyText text={workspace.repoUrl} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel="Copié">
                      <Copy className="h-3.5 w-3.5" />
                    </CopyText>
                  </div>
                ) : workspace.repoUrl ? (
              ...
              <DetailRow label="Référence de base">
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : "Aucun"}
              </DetailRow>
              <DetailRow label="Branche">
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : "Aucun"}
              </DetailRow>
              <DetailRow label="Ouvert le">{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label="Dernière utilisation">{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label="Nettoyage">
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>Journaux d'exécution et de nettoyage</CardTitle>
              <CardDescription>Opérations récentes</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">Chargement des opérations de l'espace de travail…</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : "Échec du chargement des opérations de l'espace de travail."}
              </p>
              ...
              <p className="text-sm text-muted-foreground">Aucune opération d'espace de travail n'a encore été enregistrée.</p>
            )}
            </CardContent>
          </Card>
        ) : (
